...
Setpoint Setpoint_Interrupt_Mode(Setpoint *Data, BUTTON_Name *Up, BUTTON_Name *Down, BUTTON_Name *Mode, CLCD_I2C_Name *LCD, Mode_State currentMode) {
    switch (Read_Button_Mode(currentMode, Mode)) {
        case MODE_TIME:
            CLCD_I2C_Clear(LCD);
            CLCD_I2C_SetCursor(LCD, 0, 0);
            CLCD_I2C_WriteString(LCD, "Time(minute):");
            CLCD_I2C_SetCursor(LCD, 5, 1);
            while (Read_Button_Mode(currentMode, Mode) == MODE_TIME) {
                Data_Down(&Data->time, Down, LCD, 1, 60);
                Data_Up(&Data->time, Up, LCD, 1, 60);
            }
            break;
        case MODE_HUMIDITY:
            CLCD_I2C_Clear(LCD);
            ...
            CLCD_I2C_SetCursor(LCD, 5, 1);
            while (Read_Button_Mode(currentMode, Mode) == MODE_HUMIDITY) {
                Data_Down(&Data->humidity, Down, LCD, 0, 100);
                Data_Up(&Data->humidity, Up, LCD, 0, 100);
            }
            break;
        case MODE_TEMPERATURE:
            ...
            CLCD_I2C_SetCursor(LCD, 5, 1);
            while (Read_Button_Mode(currentMode, Mode) == MODE_TEMPERATURE) {
                Data_Down(&Data->temperature, Down, LCD, 10, 40);
                Data_Up(&Data->temperature, Up, LCD, 10, 40);
            }
            break;
        default:
            break;
